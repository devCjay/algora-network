...
import { removeDeposit } from "../../Api/paymentApi"; // Import the removeOrder function
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                ...
                const deposit = await AsyncStorage.getItem("newDeposit");

                if (deposit) {

                    const parsedOrder = JSON.parse(deposit);
        ...
        if (myOrder) {
            ...
            if (myOrder.payment_status === "Completed") {
                ...
                timeoutIdRef.current = setTimeout(async () => {
                    ...
                    try {
                        await removeDeposit(); // Remove order from AsyncStorage
