...
import { ExtendMiner, useListMiner } from "../../Api/minerApi";
...
const ExtendOrderScreen = () => {
    ...
    const { minerDetails } = route.params;
    ...
    const loadState = async () => {
        try {
            const value = await AsyncStorage.getItem('minerId');
            return value;
        } catch (e) {
    ...
    // Automatically call when page renders
    useEffect(() => {
    ...
    const shortenUUID = (uuid) => {
        ...
        return parts.length === 5 ? `${parts[0]}` : "Invalid UUID";
    };


    const mid = loadedData;
    const getId = shortenUUID(mid);


    // Function to get price by matching the first part of UUID
    const getPriceByUuid = (targetUuid) => {
        if (!targetUuid) return null;

        const targetFirstPart = targetUuid.split('-')[0];
        const matchingRow = orderData.find(row => {
            const rowFirstPart = row.id?.split('-')[0];
            return rowFirstPart === targetFirstPart;
        });

        return matchingRow ? matchingRow.price : null;
    };

    // Example usage
    const minerPrice = getPriceByUuid(getId);
    console.log(minerPrice); // Output: 2.5 (from the first matching row)

    const finalPrice = minerPrice * numMonths;
    ...
    console.log(id, numMonths, currency, finalPrice);
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        const result = await ExtendMiner(id, numMonths, currency);
        setLoading(false);
        ...
        if (result.success) {
            Toast.show({
                type: "success",
                text1: "Order Created!",
                text2: "Please Make Payment",
            });

            await AsyncStorage.setItem("exOrder", JSON.stringify(result.data));
            navigation.replace("ExtendPayment");
        ...
        <>
            <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
                ...
                >
                    ...
                    <Header leftIcon="back" title="Extend Miner" />
                        ...
                        <View style={{ ...GlobalStyleSheet.container, flex: 1 }}>
                            ...
                            >
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 }}>
                                    <Text style={{ ...FONTS.h5, color: colors.text }}>Extending | {loadedData}</Text>
                                </View>
                                ...
                                <View style={{ marginBottom: 15 }}>
                                    ...
                                    >
                                        ...
                                        >
                                            <Text style={{ ...FONTS.fontBold, color: COLORS.white }}>${minerPrice}</Text>
                                        </View>
...
export default ExtendOrderScreen;
